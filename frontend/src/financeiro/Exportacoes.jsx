...
export default function Exportacoes() {
  ...
  async function exportarXmlsEntrada() {
    try {
      setErro("");
      setLoading("xmlEntrada");

      const response = await api.get(
        `/financeiro/xmls/exportar?${query}`,
        {
          responseType: "blob",
        }
      );

      const nome = usarHorario
        ? `xmls_entrada_${inicio}_${horaInicio.replace(
            ":",
            "-"
          )}_a_${fim}_${horaFim.replace(":", "-")}.zip`
        : `xmls_entrada_${inicio}_a_${fim}.zip`;
      ...
      let mensagem = "Erro ao exportar XMLs de entrada";
...
`;
      ...
      if (!w) {
        alert(
          "Pop-up bloqueado. Libere pop-up para imprimir."
        );
