...
function calcularFechamento({ abertura, dinheiro, pix, cartao, entradas, saidas, totalDeclarado }) {
  const totalCalculado =
    n(abertura) + n(dinheiro) + n(pix) + n(cartao) + n(entradas) - n(saidas);

  const diferenca = n(totalDeclarado) - totalCalculado;

  return {
    totalCalculado,
    totalDeclarado: n(totalDeclarado),
    diferenca,
  };
}
...
export default function FechamentoCaixa() {
  ...
  async function carregar() {
    ...
    try {
      ...
      const hist = await api.get("/caixa/fechamentos?limit=50&page=1");
  ...
  const totalPaginas = useMemo(() => {
    return Math.max(1, Math.ceil(historico.length / porPagina));
  }, [historico.length]);

  const historicoPaginado = useMemo(() => {
    const inicio = (histPage - 1) * porPagina;
    return historico.slice(inicio, inicio + porPagina);
  }, [historico, histPage]);
        ...
        {!sessao ? (
          ...
          >
            ...
            <div className="panel">
              ...
              >
                <div>
                  <div className="mk-selected-k">Dinheiro contado</div>
                  <input
                    value={dinheiroDecl}
                ...
                <div>
                  <div className="mk-selected-k">PIX conferido</div>
                  <input
                    value={pixDecl}
                ...
                <div>
                  <div className="mk-selected-k">Cartão conferido</div>
                  <input
                    value={cartaoDecl}
        ...
        {!historicoPaginado.length ? (
          ...
          >
            {historicoPaginado.map((item) => {
              const abertura = n(item.valor_abertura);

              const dinheiro = n(item.dinheiro);
              const pix = n(item.pix);

              const cartao =
                n(item.cartao) + n(item.credito) + n(item.debito);

              const entradas = n(item.entradas);
              const saidas = n(item.saidas);

              const declaradoDinheiro = n(item.valor_fechamento) - abertura;
              const declaradoPix = 0;
              const declaradoCartao = 0;
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className="panel">
                      <div className="mk-selected-k">Dinheiro</div>
                      <div className="mk-selected-v">{money(dinheiro)}</div>
                    </div>

                    <div className="panel">
                      <div className="mk-selected-k">PIX</div>
                      <div className="mk-selected-v">{money(pix)}</div>
                    </div>

                    <div className="panel">
                      <div className="mk-selected-k">Cartão</div>
                      <div className="mk-selected-v">{money(cartao)}</div>
                    </div>
                    ...
                    <div className="panel">
                      ...
                      <div
                        className="mk-selected-v"
                        style={{
                          color: Number(diferenca || 0) === 0 ? "#2ecc71" : "#ff7675",
                        }}
                      >
                        {money(diferenca)}
                      </div>
